...
var defaultOptions = {
    ...
    resolveToken: function (response, config) {
        ...
        var /** @type {?} */ tokenRootData = config.tokenRoot && config.tokenRoot
            .split('.')
            .reduce(function (o, x) {
            return o[x];
        }, accessToken);
...
var ConfigService = (function () {
    ...
    function ConfigService(options) {
        this.options = Object.assign({}, defaultOptions, options, { providers: Object.assign({}, options.providers, Object
                .keys(defaultOptions.providers)
                .concat(Object.keys(options.providers || {}))
                .map(function (key) {
                return options.providers && options.providers[key]
                    ? (_d = {}, _d[key] = Object.assign({}, defaultOptions.providers[key], options.providers[key]), _d) : (_e = {}, _e[key] = defaultOptions.providers[key], _e);
                var _d, _e;
            })
                .reduce(function (acc, next) { return (Object.assign({}, acc, next)); }, {})) });
    }
...
var BrowserStorageService = (function (_super) {
    ...
    BrowserStorageService.prototype.updateStorageType = function (storageType) {
        var /** @type {?} */ isStorageAvailable = this.checkIsStorageAvailable(storageType);
        ...
        return __awaiter$1(this, void 0, void 0, function () {
            return __generator(this, function (_d) {
                switch (this.storageType) {
                    case COOKIE:
                    case SESSION_COOKIE:
                        return [2 /*return*/, this.getCookie(key)];
                    case LOCAL_STORAGE:
                    case SESSION_STORAGE:
                        return [2 /*return*/, window[this.storageType].getItem(key)];
                    case MEMORY:
    ...
    BrowserStorageService.prototype.isWindowStorageAvailable = function (storageType) {
        try {
            var /** @type {?} */ supported = window && storageType in window && window[storageType] !== null;
            if (supported) {
...
var SharedService = (function () {
        ...
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_e) {
                switch (_e.label) {
                    ...
                    case 2:
                        token = _d;
                        return [4 /*yield*/, this.getPayload(token)];
                    case 3:
                        payload = _e.sent();
    ...
    SharedService.prototype.logout = function () {
        ...
        return Observable.Observable.create(function (observer) {
            _this.storage.remove(_this.tokenName).then(function () {
                observer.next();
                observer.complete();
            });
        });
...
var PopupService = (function () {
    ...
    PopupService.prototype.open = function (url, options, cordova) {
        var /** @type {?} */ stringifiedOptions = this.stringifyOptions(this.prepareOptions(options.popupOptions));
        var /** @type {?} */ UA = window.navigator.userAgent;
        cordova = cordova === null ? this.isCordovaApp() : cordova;
        var /** @type {?} */ windowName = cordova ? '_blank' : options.name;
        var /** @type {?} */ popupWindow = window.open(url, windowName, stringifiedOptions);
    ...
    PopupService.prototype.eventListener = function (popupWindow, redirectUri) {
        ...
        return merge.merge(fromEvent.fromEvent(popupWindow, 'exit').pipe(operators.delay(100), operators.map(function () { throw new Error('Authentication Canceled'); })), fromEvent.fromEvent(popupWindow, 'loadstart')).pipe(operators.switchMap(function (event) {
            if (!popupWindow || popupWindow.closed) {
                return Observable.Observable.throw(new Error('Authentication Canceled'));
            }
            ...
            if (parser.search || parser.hash) {
                ...
                var /** @type {?} */ allParams = Object.assign({}, qs, hash);
                popupWindow.close();
                if (allParams.error) {
                    throw allParams.error;
                }
                else {
                    return of.of(allParams);
                }
            }
            return empty.empty();
        }), operators.take(1));
    ...
    PopupService.prototype.pollPopup = function (popupWindow, redirectUri) {
        ...
        return interval.interval(50)
            ...
            if (!popupWindow || popupWindow.closed) {
                return _throw._throw(new Error('Authentication Canceled'));
            }
            var /** @type {?} */ popupWindowOrigin = getWindowOrigin(popupWindow);
            if (popupWindowOrigin &&
                ...
                var /** @type {?} */ queryParams = popupWindow.location.search.substring(1).replace(/\/$/, '');
                var /** @type {?} */ hashParams = popupWindow.location.hash.substring(1).replace(/[\/$]/, '');
                var /** @type {?} */ hash = _this.parseQueryString(hashParams);
                var /** @type {?} */ qs = _this.parseQueryString(queryParams);
                popupWindow.close();
                var /** @type {?} */ allParams = Object.assign({}, qs, hash);
...
var Oauth1Service = (function () {
    ...
    Oauth1Service.prototype.exchangeForToken = function (oauthOptions, authorizationData, oauthData, userData) {
        var /** @type {?} */ body = { authorizationData: authorizationData, oauthData: oauthData, userData: userData };
        var _d = this.config.options, withCredentials = _d.withCredentials, baseUrl = _d.baseUrl;
        var _e = oauthOptions.method, method = _e === void 0 ? 'POST' : _e, url = oauthOptions.url;
        var /** @type {?} */ exchangeForTokenUrl = baseUrl ? joinUrl(baseUrl, url) : url;
...
var Oauth2Service = (function () {
    ...
    Oauth2Service.prototype.open = function (oauthOptions, userData) {
        var _this = this;
        var /** @type {?} */ authorizationData = this.getAuthorizationData(oauthOptions);
        var /** @type {?} */ url = [oauthOptions.authorizationEndpoint, buildQueryString(authorizationData)].join('?');
    ...
    Oauth2Service.prototype.exchangeForToken = function (options, authorizationData, oauthData, userData) {
        var /** @type {?} */ body = { authorizationData: authorizationData, oauthData: oauthData, userData: userData };
        var _d = this.config.options, baseUrl = _d.baseUrl, withCredentials = _d.withCredentials;
        var url = options.url, _e = options.method, method = _e === void 0 ? 'POST' : _e;
        var /** @type {?} */ exchangeForTokenUrl = baseUrl ? joinUrl(baseUrl, url) : url;
        return this.http.request(method, exchangeForTokenUrl, { body: body, withCredentials: withCredentials });
    };
    /**
    ...
    Oauth2Service.prototype.getAuthorizationData = function (options) {
        var _d = options.responseType, responseType = _d === void 0 ? 'code' : _d, clientId = options.clientId, _e = options.redirectUri, redirectUri = _e === void 0 ? getWindowOrigin() || '' : _e, _f = options.scopeDelimiter, scopeDelimiter = _f === void 0 ? ',' : _f, scope = options.scope, state = options.state, additionalUrlParams = options.additionalUrlParams;
        var /** @type {?} */ resolvedState = typeof state === 'function' ? state() : state;
        return [
            ...
            ? Object
                ...
                .map(function (key) {
                var /** @type {?} */ value = ((additionalUrlParams))[key];
            ...
            return (Object.assign({}, acc, (_d = {}, _d[next[0]] = next[1], _d)));
            var _d;
        }, /** @type {?} */ ({}));
    };
...
function observeOn(scheduler, delay$$1) {
    ...
    return function observeOnOperatorFunction(source) {
        return source.lift(new ObserveOnOperator(scheduler, delay$$1));
    };
}
var observeOn_2 = observeOn;
var ObserveOnOperator = (function () {
    function ObserveOnOperator(scheduler, delay$$1) {
        if (delay$$1 === void 0) {
            delay$$1 = 0;
        }
        this.scheduler = scheduler;
...
var ObserveOnSubscriber = (function (_super) {
    ...
    function ObserveOnSubscriber(destination, scheduler, delay$$1) {
        if (delay$$1 === void 0) {
            delay$$1 = 0;
        }
        _super.call(this, destination);
...
Observable_1.Observable.empty = empty$5.empty;
...
Observable_1.Observable.fromEvent = fromEvent$4.fromEvent;
...
var Action = (function (_super) {
     ...
     * @return {void}
     */
    Action.prototype.schedule = function (state, delay$$1) {
        if (delay$$1 === void 0) {
            delay$$1 = 0;
        }
        return this;
...
var AsyncAction = (function (_super) {
    ...
    AsyncAction.prototype.schedule = function (state, delay$$1) {
        if (delay$$1 === void 0) {
            delay$$1 = 0;
        }
        if (this.closed) {
        ...
        //
        if (id != null) {
            this.id = this.recycleAsyncId(scheduler, id, delay$$1);
        }
        this.delay = delay$$1;
        // If this action has already an async Id, don't request a new one.
        this.id = this.id || this.requestAsyncId(scheduler, this.id, delay$$1);
        return this;
    };
    AsyncAction.prototype.requestAsyncId = function (scheduler, id, delay$$1) {
        if (delay$$1 === void 0) {
            delay$$1 = 0;
        }
        return root.root.setInterval(scheduler.flush.bind(scheduler, this), delay$$1);
    };
    AsyncAction.prototype.recycleAsyncId = function (scheduler, id, delay$$1) {
        if (delay$$1 === void 0) {
            delay$$1 = 0;
        }
        // If this action is rescheduled with the same delay time, don't clear the interval id.
        if (delay$$1 !== null && this.delay === delay$$1 && this.pending === false) {
            return id;
        }
     ...
     * @return {any}
     */
    AsyncAction.prototype.execute = function (state, delay$$1) {
        if (this.closed) {
    ...
    AsyncAction.prototype._execute = function (state, delay$$1) {
        var errored = false;
...
var Scheduler$1 = (function () {
     ...
     * the scheduled work.
     */
    Scheduler.prototype.schedule = function (work, delay$$1, state) {
        if (delay$$1 === void 0) {
            delay$$1 = 0;
        }
        return new this.SchedulerAction(this, work).schedule(state, delay$$1);
    };
...
Observable_1.Observable.interval = interval$4.interval;
/* tslint:enable:max-line-length */
function merge$7() {
    var observables = [];
    for (var _i = 0; _i < arguments.length; _i++) {
...
Observable_1.Observable.throw = _throw$4._throw;
...
var QueueAction = (function (_super) {
    ...
    QueueAction.prototype.schedule = function (state, delay$$1) {
        if (delay$$1 === void 0) {
            delay$$1 = 0;
        }
        if (delay$$1 > 0) {
            return _super.prototype.schedule.call(this, state, delay$$1);
        }
        this.delay = delay$$1;
        this.state = state;
    ...
    QueueAction.prototype.execute = function (state, delay$$1) {
        return (delay$$1 > 0 || this.closed) ?
            _super.prototype.execute.call(this, state, delay$$1) :
            this._execute(state, delay$$1);
    };
    QueueAction.prototype.requestAsyncId = function (scheduler, id, delay$$1) {
        if (delay$$1 === void 0) {
            delay$$1 = 0;
        }
        ...
        if ((delay$$1 !== null && delay$$1 > 0) || (delay$$1 === null && this.delay > 0)) {
            return _super.prototype.requestAsyncId.call(this, scheduler, id, delay$$1);
        }
...
function concat$5() {
    ...
    return concat_2$3.concat.apply(void 0, observables)(this);
}
var concat_3 = concat$5;
var concat_2$1 = {
    concat: concat_3
};
 ...
 * @owner Observable
 */
function delay$4(delay$$1, scheduler) {
    if (scheduler === void 0) {
        scheduler = async.async;
    }
    var absoluteDelay = isDate_1.isDate(delay$$1);
    var delayFor = absoluteDelay ? (+delay$$1 - scheduler.now()) : Math.abs(delay$$1);
    return function (source) { return source.lift(new DelayOperator(delayFor, scheduler)); };
}
var delay_2$2 = delay$4;
var DelayOperator = (function () {
    function DelayOperator(delay$$1, scheduler) {
        this.delay = delay$$1;
        this.scheduler = scheduler;
    }
...
var DelaySubscriber = (function (_super) {
    ...
    function DelaySubscriber(destination, delay$$1, scheduler) {
        _super.call(this, destination);
        this.delay = delay$$1;
        this.scheduler = scheduler;
        this.queue = [];
 ...
 * @owner Observable
 */
function delay$3(delay$$1, scheduler) {
    if (scheduler === void 0) {
        scheduler = async.async;
    }
    return delay_1.delay(delay$$1, scheduler)(this);
}
var delay_3 = delay$3;
var delay_2 = {
    delay: delay_3
 ...
 * @owner Observable
 */
function observeOn$3(scheduler, delay$$1) {
    if (delay$$1 === void 0) {
        delay$$1 = 0;
    }
    return observeOn_1.observeOn(scheduler, delay$$1)(this);
}
...
var AsapAction = (function (_super) {
    ...
    AsapAction.prototype.requestAsyncId = function (scheduler, id, delay$$1) {
        if (delay$$1 === void 0) {
            delay$$1 = 0;
        }
        // If delay is greater than 0, request as an async action.
        if (delay$$1 !== null && delay$$1 > 0) {
            return _super.prototype.requestAsyncId.call(this, scheduler, id, delay$$1);
        }
    ...
    AsapAction.prototype.recycleAsyncId = function (scheduler, id, delay$$1) {
        if (delay$$1 === void 0) {
            delay$$1 = 0;
        }
        // If delay exists and is greater than 0, or if the delay is null (the
        ...
        // action, then recycle as an async action.
        if ((delay$$1 !== null && delay$$1 > 0) || (delay$$1 === null && this.delay > 0)) {
            return _super.prototype.recycleAsyncId.call(this, scheduler, id, delay$$1);
        }
...
var SubscribeOnObservable = (function (_super) {
    ...
    SubscribeOnObservable.create = function (source, delay$$1, scheduler) {
        if (delay$$1 === void 0) {
            delay$$1 = 0;
        }
        if (scheduler === void 0) {
        ...
        return new SubscribeOnObservable(source, delay$$1, scheduler);
    };
    ...
    SubscribeOnObservable.prototype._subscribe = function (subscriber) {
        var delay$$1 = this.delayTime;
        var source = this.source;
        var scheduler = this.scheduler;
        return scheduler.schedule(SubscribeOnObservable.dispatch, delay$$1, {
 ...
 * @owner Observable
 */
function subscribeOn$3(scheduler, delay$$1) {
    if (delay$$1 === void 0) {
        delay$$1 = 0;
    }
    return function subscribeOnOperatorFunction(source) {
        return source.lift(new SubscribeOnOperator(scheduler, delay$$1));
    };
}
var subscribeOn_2$2 = subscribeOn$3;
var SubscribeOnOperator = (function () {
    function SubscribeOnOperator(scheduler, delay$$1) {
        this.scheduler = scheduler;
        this.delay = delay$$1;
    }
 ...
 * @owner Observable
 */
function subscribeOn$2(scheduler, delay$$1) {
    if (delay$$1 === void 0) {
        delay$$1 = 0;
    }
    return subscribeOn_1.subscribeOn(scheduler, delay$$1)(this);
}
...
var TimeInterval$2 = (function () {
    function TimeInterval(value, interval$$1) {
        this.value = value;
...
var VirtualAction = (function (_super) {
    ...
    VirtualAction.prototype.schedule = function (state, delay$$1) {
        if (delay$$1 === void 0) {
            delay$$1 = 0;
        }
        if (!this.id) {
    ...
    VirtualAction.prototype.requestAsyncId = function (scheduler, id, delay$$1) {
        if (delay$$1 === void 0) {
            delay$$1 = 0;
        }
    ...
    VirtualAction.prototype.recycleAsyncId = function (scheduler, id, delay$$1) {
        if (delay$$1 === void 0) {
            delay$$1 = 0;
        }
        return undefined;
    };
    VirtualAction.prototype._execute = function (state, delay$$1) {
        if (this.active === true) {
            return _super.prototype._execute.call(this, state, delay$$1);
        }
    };
...
var AnimationFrameAction = (function (_super) {
    ...
    AnimationFrameAction.prototype.requestAsyncId = function (scheduler, id, delay$$1) {
        if (delay$$1 === void 0) {
            delay$$1 = 0;
        }
        // If delay is greater than 0, request as an async action.
    ...
    AnimationFrameAction.prototype.recycleAsyncId = function (scheduler, id, delay$$1) {
        if (delay$$1 === void 0) {
            delay$$1 = 0;
        }
        // If delay exists and is greater than 0, or if the delay is null (the
...
var Observable$2 = Observable_1.Observable;
/**
...
var OauthService = (function () {
    ...
    OauthService.prototype.authenticate = function (name, userData) {
        var _this = this;
        var /** @type {?} */ provider = this.providerOf(name);
        return provider.open(this.config.options.providers[name], userData || {})
            ...
            if (_this.config.options.providers[name].url) {
                return Observable$2.from(_this.shared.setToken(response)).map(function () { return response; });
            }
            return Observable$2.of(response);
        }));
    };
    /**
     * @param {?} name
     * @return {?}
     */
    OauthService.prototype.providerOf = function (name) {
        return this.config.options.providers[name].oauthType === '1.0'
            ? core.Injector.create(this.depProviders.concat([
...
var JwtInterceptor = (function () {
    ...
    JwtInterceptor.prototype.intercept = function (req, next) {
        var _d = this.config.options, authHeader = _d.authHeader, authToken = _d.authToken;
        return Observable.Observable
            .fromPromise(Promise.all([this.shared.getToken(), this.shared.isAuthenticated()]))
            .switchMap(function (auth) {
            var /** @type {?} */ token = auth[0];
            var /** @type {?} */ isAuthenticated = auth[1];
            var /** @type {?} */ newReq = isAuthenticated && !req.headers.has(authHeader)
                ? req.clone({ setHeaders: (_d = {}, _d[authHeader] = authToken + " " + token, _d) })
                : req;
